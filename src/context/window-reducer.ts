/**
 * The window manager's state machine.
 *
 * Split out of the provider so it can be exercised without React: it is a pure
 * function of (state, action), and it holds the trickiest logic in the shell —
 * snap geometry, tearing a window off a snap, and what "restore" means after
 * several snaps in a row. See window-reducer.test.ts.
 */
...
export type State = { windows: WindowState[]; nextZ: number };
