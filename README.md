...
| Apps | Tips, About (System Properties), File Explorer, Skills, Experience, Mail (Contact), Media Player, Settings, VS Code, Recycle Bin, Notes, Terminal, Career.exe |
| Career.exe | The five roles as a proportional track above a side-scroller: the career is legible before a step is taken, and clicking a role walks the character to that role's signpost. The same track, static, heads the Summary tab |
| Tips | Opens by itself on a first visit and never again — snapped beside About, so the person is on screen and the two-window layout shows snapping before anyone reads about it. `F1` reopens it. Four pages: the gestures, the keyboard map, the five decisions the shell is built on, and what is deliberately absent |
